...
printf '| Repo | Vis | Tier | Renovate | Silver (scorecard) | Gold (release) | Next rung |\n'
printf '|---|---|---|---|---|---|---|\n'
...
while IFS= read -r repo; do
  ...
  if has_caller "$callers" "release-please"; then gold="✅"; else gold="—"; fi
  ...
  elif [ "$tier" != "docs" ] && [ "$gold" = "—" ]; then next="Gold: add release-please + release-artifacts";
  fi
  ...
  printf '| %s | %s | %s | %s | %s | %s | %s |\n' \
    "$repo" "$vis" "$tier" "$renov" "$silver" "$gold" "$next"
