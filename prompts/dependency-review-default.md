...
**7. Repo invariants disturbed.** Some changes are individually correct and still leave the repository in a broken state, and the bot has no mechanism to fix it. In `SpiceLabsHQ/.github` specifically: a bump to a pinned action SHA — or to a `with:` version Renovate reads, such as `python-version` — inside a reusable workflow under `.github/workflows/` also has to move the mirrored entry in that workflow's `workflows/<name>/pins.yml`, because release-please routes commits by directory path and a bump that reaches no package cuts no release. That mirror is what to check. The `workflows/<name>/workflow.sha256` beside it is not: nothing reads its value, it is the affordance a human uses to route a hand edit, and on a bot bump it is left stale on purpose — a stale checksum there is the expected state, not a finding. More generally: a pinning convention the bump breaks, a version enumerated in an allowlist or preset that now disagrees, a generated file whose source changed. If the invariant is visible in the diff it is yours to flag; if it is only visible by running a script, name it as an unverified gap rather than asserting either way.
