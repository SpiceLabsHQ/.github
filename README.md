...
- **Bots, by default** — but only an **enumerated allowlist** (`renovate[bot]`, `dependabot[bot]`), never a `*[bot]` glob (every GitHub App login ends in `[bot]`). Bots arm automatically because no human reads Pepper's feedback on a bot's behalf.
  - That allowlist names PR **authors**. Pepper's `allowed_bots` names event **initiators** — a different list, deliberately: the stranded-PR sweep (DEV-667) reopens PRs as Pepper's own App, so `pepper-pr-review` is an initiator and must never become an author. Actions has no cross-workflow literal sharing, so the two lists are duplicated across three files and [`scripts/check-bot-allowlists.sh`](scripts/check-bot-allowlists.sh) asserts they agree on every PR (`repo-checks.yml`). Add a new dependency bot to **both** `pepper-pr-review.yml`'s `allowed_bots` default and `floor-automerge.yml`'s author array, or the check fails and tells you which side is missing.
...
- **Enabling auto-merge** — [`floor-automerge.yml`](.github/workflows/floor-automerge.yml) arms GitHub native auto-merge (squash) on code-tier PRs, injected zero-install by the `spice-ci-floor-code` ruleset. The author-class test lives in the **job-level `if:`**, so a non-arming PR (a human PR in a non-opt-in repo, or an unlisted bot) **skips with zero runner minutes** — a skipped required workflow counts as *passing*, so it never deadlocks merge. Opt-in is read from the payload at `github.event.repository.custom_properties['automerge-humans']` (hyphenated → bracket syntax; values are strings). It is best-effort (never fails its required check) and also skips drafts, this `.github` repo, and `rosemary-releaser` release PRs (the rosemary skip is *kept*, not redundant — the opt-in clause is repo-scoped and would otherwise arm a release PR in an opt-in repo). It arms as the `spice-automerge` App rather than `GITHUB_TOKEN` so the eventual merge push *does* fire `push` / `workflow_run` workflows (the DEV-514 deploy-trigger gap); the App's Client ID is a literal in the workflow and `AUTOMERGE_APP_PRIVATE_KEY` must exist in **both** org secret stores, or Dependabot PRs silently fall back to `GITHUB_TOKEN` and skip deploys (DEV-902 — see the both-stores note below).
- **Renovate PRs** self-enable auto-merge natively — a **second, consistent bots-by-default arming path**. The shared preset [`default.json`](default.json) (consumed via `"extends": ["github>SpiceLabsHQ/.github"]`) sets `platformAutomerge` + `automerge` on all update types, groups non-major updates into one PR (fewer Pepper runs, no lockfile-conflict cascade), and applies a `minimumReleaseAge` of 7 days as a supply-chain soak (DEV-671 — long enough that upstream point releases settle before a PR opens, instead of churning the branch mid-review). Vulnerability-driven updates are exempt from the soak via the `vulnerabilityAlerts` object, which clears `minimumReleaseAge` for those updates only. That object sources from **Dependabot alerts** and is the load-bearing half — it is the only path that covers **GitHub Actions**, the org's dominant dependency class. `osvVulnerabilityAlerts` is declared as a supplement for other ecosystems, not a replacement: Renovate's OSV ecosystem map carries no `github-tags` entry, so an OSV-only carve-out would leave action CVEs behind the full soak. **Per-repo dependency:** the exemption depends on the *consuming* repo having Dependabot alerts enabled, which a shared preset cannot guarantee — where they are off, action CVEs get no exemption and the carve-out silently matches nothing. That is why alerts are enabled org-wide and scored per repo in the audit's `Dep-alerts` column (DEV-1142) rather than assumed.
...
- **After editing any reusable workflow, run [`scripts/sync-workflow-checksums.sh`](scripts/sync-workflow-checksums.sh)** and commit the regenerated `workflows/<name>/workflow.sha256`. GitHub forces all workflow YAML into the flat `.github/workflows/` directory, while release-please routes commits to packages by directory path — the checksum file is the bridge that routes your commit to the right package. The same applies to files a workflow ships from outside its YAML: `pepper-pr-review` reads its review prompts from `prompts/` at the release commit, so those templates are hashed into its checksum file too and a prompt-only edit still cuts a release. `repo-checks.yml` fails any PR that changes a reusable workflow — or a file it ships — without also changing something under `workflows/<name>/`, so you can't forget silently. Note what that does and doesn't assert: it checks that your commit **routes**, not that the stored hash is current. Nothing ever reads the hash, so a checksum whose workflow was last touched by a bot bump can sit stale on `main` indefinitely; that's expected, not a bug.
- **`workflows/<name>/pins.yml` will be the same bridge for bots**, and the same script writes it. It mirrors every dependency in the workflow that Renovate can bump — the `uses:` refs *and* the `with:` values Renovate reads as versions, such as `python-version` — and `.github/renovate.json` points Renovate at those paths, so a bump manages the same dependency in two places and its own commit already contains a file under `workflows/<name>/`: no human touch, no red routing check on a bot PR. **Not in effect yet:** DEV-1313 added the generator, DEV-1314 commits the twelve files, and until then running the sync script just leaves them untracked. GitHub Actions never runs the file; the composite-action shape is deliberate, because that shape's schema has no runner concept and so cannot invent a dependency no workflow has. Don't hand-edit it — `scripts/sync-workflow-checksums.sh --check` will tell you it disagrees with the workflow. See [`workflows/README.md`](workflows/README.md#why-pinsyml-exists-do-not-edit-by-hand).
...
**Org standards (DEV-1119).** Every default-flavor review also carries the index of the [Eng-Cookbook](https://github.com/SpiceLabsHQ/Eng-Cookbook) `standards/` at the cookbook's **latest stable release** — never its `main` or a prerelease; being in a cut release is the enforceability line. The workflow mints a second App token down-scoped to `Eng-Cookbook` (contents: read), resolves the tag, sparse-checks `standards/` out to `.pepper-standards/` with `persist-credentials: false`, and drops the token before the model starts — the review token is unchanged and the agent never holds a cookbook credential. Pepper routes from the diff to the two or three standards it implicates and cites findings as standard + rule number + level (MUST blocks; SHOULD with a stated reason is a note; MAY never blocks); a repo's `.pepper/pr-review-standards.md` still wins on conflict. Fail-open: no stable release or a failed checkout injects a "no org standards this run" marker and the review proceeds off the diff. The resolved tag is recorded as `cookbook_ref` in the [audit record](docs/pepper-audit.md).
...
> **Both stores, or Dependabot PRs deadlock.** GitHub keeps org **Actions** secrets and org **Dependabot** secrets in two separate stores, and a Dependabot-triggered run reads only the Dependabot one — an Actions-scoped secret interpolates to an empty string there no matter its visibility. Every secret above must therefore be set **twice**. Pepper failed on every genuinely Dependabot-triggered run for months because the private key existed only under Actions; since `floor-pepper.yml` is a required check, the affected PRs could not be merged at all (DEV-797). The hazard is not confined to this workflow: `floor-automerge.yml` hit the same wall with `AUTOMERGE_APP_PRIVATE_KEY` and, because its token mint is best-effort, degraded *silently* to `GITHUB_TOKEN` instead of failing — trading a deadlock for months of skipped deploys (DEV-902). Any org secret read on a Dependabot-triggered path needs both copies. Set the Dependabot copies with `--app dependabot`:
>
> ```bash
