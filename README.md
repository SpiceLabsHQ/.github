...
- **`workflows/<name>/pins.yml` is the same bridge for bots**, and the same script writes it. It mirrors every `uses:` ref in the workflow that Renovate can bump, and `.github/renovate.json` points Renovate at those paths — so a bump manages the same ref in two places and its own commit already contains a file under `workflows/<name>/`. No human touch, no red routing check on a bot PR. GitHub Actions never runs the file; it's shaped as a composite action only because that's what Renovate parses most cleanly. Don't hand-edit it — `scripts/sync-workflow-checksums.sh --check` will tell you it disagrees with the workflow. See [`workflows/README.md`](workflows/README.md#why-pinsyml-exists-do-not-edit-by-hand).
